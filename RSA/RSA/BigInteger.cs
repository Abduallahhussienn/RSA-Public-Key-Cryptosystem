...
namespace RSA
{
    class BigInteger:IComparable<BigInteger>,IEquatable<BigInteger>
    {
        ...
        {
            string first = X.Number_getter();
            string second = Y.Number_getter();
            int len1 = first.Length;
            int len2 = second.Length;
            int n = 0;
            int numberofzeros = GetNumberofzeros(len1, len2);
            // Base cases
            /*         if (n == 0)
                     {
                         int i = 0;
                         string ii = i.ToString();
                         BigInteger bb = new BigInteger(ii);
                         return bb;
                     }*/
            if (n == 1)
            {
                string f = X.Number_getter();
                string s = Y.Number_getter();
                BigInteger b = new BigInteger(multiplyiSingleBit(first, second));
                return b;
            }
            ...
            if (len1 > len2)
            {
                Y = PadWithZeros(Y, numberofzeros);
                n = Y.Number.Length;
            }
            else if (len2 > len1)
            {
                X = PadWithZeros(X, numberofzeros);
                n = X.Number.Length;
            }
            else
            {
            ...
            BigInteger A = new BigInteger(first.Substring(0, first_half));
            BigInteger B = new BigInteger(first.Substring(first_half, second_half));
            BigInteger C = new BigInteger(second.Substring(0, first_half));
            BigInteger D = new BigInteger(second.Substring(first_half, second_half));
            ...
            return PadWithZeros(M2, n) + PadWithZeros(M3 - M1 - M2, n) + M1;
            throw new NotImplementedException();
        }



        static int multiplyiSingleBit(string a, string b)
        { return (a[0] - '0') * (b[0] - '0'); }

        private static int GetNumberofzeros(int len1, int len2)
        {
            if (len1 < len2)
            {
                return len2 - len1;
